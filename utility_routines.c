...
/*
 *
 *  Description: put formated string to current SYS$OUTPUT
 *
 *  Input:
 *	fmt:	ptintf's format specefication
 *	...	arguments
 *
 *  Return:
 *	SS$_NORMAL
 ...
 *
 */
const char severity[STS$K_INFO + 1]= { 'W', 'S', 'E', 'I'};
...
struct timespec now;
	...
	** Some sanity check
	*/
	if ( !($ISINRANGE(sev, STS$K_WARN, STS$K_INFO)) )
		return	sev;
...
{
va_list arglist;
const char	lfmt [2][64] = {
			{"%02u-%02u-%04u %02u:%02u:%02u.%03u " PID_FMT " [%s\\%u] %%%s-%C:"},
			{"%02u-%02u-%04u %02u:%02u:%02u.%03u " PID_FMT " [%s\\%u] <%08X>%%"}
			},
		mfmt [2][64] = {
			{"%02u-%02u-%04u %02u:%02u:%02u.%03u " PID_FMT " [%s\\%s\\%u] %%%s-%C:"},
			{"%02u-%02u-%04u %02u:%02u:%02u.%03u " PID_FMT " [%s\\%s\\%u] <%08X> "}
			};
char	out[1024];
unsigned olen, sev = $SEV(msgno), opcom = msgno & STS$M_SYSLOG, fmtidx = 0;
struct tm _tm = {0};
struct timespec now = {0};
	...
	*/
	...
	if ( !fmtidx )
		...
		olen = __mod
			? snprintf (out, sizeof(out), mfmt[1], _tm.tm_mday, _tm.tm_mon + 1, 1900 + _tm.tm_year,
				_tm.tm_hour, _tm.tm_min, _tm.tm_sec, (unsigned) now.tv_nsec/TIMSPECDEVIDER,
				(unsigned) gettid(), __mod, __fi, __li, msgno)
			: snprintf (out, sizeof(out), lfmt[1], _tm.tm_mday, _tm.tm_mon + 1, 1900 + _tm.tm_year,
				_tm.tm_hour, _tm.tm_min, _tm.tm_sec, (unsigned) now.tv_nsec/TIMSPECDEVIDER,
				(unsigned) gettid(), __fi, __li, msgno);
		}
...
char **	strings;
	...
	fprintf(stdout, " --------------------- TRACEBACK BEGIN (%d) ------", nptrs);
	fflush(stdout);
