...
enum {
	...
	SWARM$K_REQ_CB_DOWN,				/* Contorl Block instance is shutting down */


	SWARM$K_REQ_CB_CHECK,				/* Contorl Block request data set from Client */
...
typedef	struct  __swarm_pdu__ {
	...
	unsigned short	len;				/* NBO, A length of the whole PDU (header + payload) in octets */
...
#define	SWARM$SZ_PDUHDR	(offsetof(struct __swarm_pdu_hdr__, data))
